...
const CoursesMain = ({ RefObj, setIsOpen }) => {
  ...
  const onSubmitCourse = async (values) => {
    ...
    if (editing) {
      body.id = editing.id;
      try {
        const { data } = await AdminProvider.updateCourse(editing.id, body);
      ...
      AdminProvider.createCourse(body)
      .then(res=>{
        reset();
          setForRender(Math.random());
  ...
  const handleDeleteCourse = (obj) => {
    ...
    new Promise((res, rej) => {
      ...
      .then(async () => {
        await AdminProvider.deleteCourse(obj.id);
        setCourse((p) =>
          p.filter((teach) => {
    ...
    return (
       ...
       >
         ...
         >
           ...
           />
           <br />
           <button type="submit" className="btn btn-primary" style={{display:"flex"}}>
